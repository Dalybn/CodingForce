...
int liste_absences (Absence a){
    ...
    f=fopen("absence.txt","r");
    f1=fopen("listeabs.txt","a+");
    f2=fopen("utilisateur.txt","r");
    while(fscanf(f2,"%d %s %s %s %d %s %d %s %s %s\n",&id1,nom,prenom,sexe,&jour1,mois1,&annee1,cin,telephone,adresse)!=EOF){
        while(fscanf(f," %d %d %s %d %s\n",&ID,&jour,mois,&annee,raison)!=EOF){
            if(id1==ID){
                nbr+=1;
            }  
...
int id;
int idmin;
int nbrminabs = liste_absences (a);
char nom[20];
char prenom[20];
char sexe[20];
int jour;
...
f=fopen("utilisateur.txt","r");
f1=fopen("meilleurouvrier.txt","a");
f2=fopen("listeabs.txt","r");
...
////fonction affichage meilleur ouvrier ////
