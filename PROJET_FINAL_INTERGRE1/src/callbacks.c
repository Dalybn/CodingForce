...
while(fscanf(f,"%d %s %s %d %s %d\n",&A.identifiant,A.type,A.sexe,&A.d.jour,A.d.mois,&A.d.annee)!=EOF)
	{
//gtk_combo_box_set_active (GTK_COMBO_BOX(mois),y);
...
sortie=lookup_widget(objet,"label661");
input1=lookup_widget(objet,"REFERENCE");
strcpy(ref,gtk_entry_get_text(GTK_ENTRY(input1)));
trouve=rechercher_capt(c,ref);
...
gdk_color_parse("green",&color);
gtk_widget_modify_fg(sortie,GTK_STATE_NORMAL,&color);
gtk_label_set_text(GTK_LABEL(sortie),"Capteur trouvé");
}
else
{
GdkColor color;
...
sortie=lookup_widget(objet,"label663");

strcpy(id,gtk_combo_box_get_active_text(GTK_COMBO_BOX(input)));
 
trouve=rechercher_reclamation(id);
...
gdk_color_parse("green",&color);
gtk_widget_modify_fg(sortie,GTK_STATE_NORMAL,&color);
gtk_label_set_text(GTK_LABEL(sortie),"reclamation trouvé");
}
else
{
GdkColor color;
...
sortie=lookup_widget(objet,"label278");
strcpy(r.identifiant,gtk_combo_box_get_active_text(GTK_COMBO_BOX(input1)));
//strcpy(r.identifiant,gtk_entry_get_text(GTK_ENTRY(input1)));
r.d.jour=gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON (jour));
...
if(gtk_tree_model_get_iter(model,&iter,path))
{
gtk_tree_model_get (GTK_LIST_STORE(model), &iter,0,id_recl,1,&identifiant,2,&jour,3,&mois,4,&annee,5,&reclamation,6,&etat,7,&rep,-1);
r.id_recl=id_recl;
...
fenetre_afficher_reclamation=lookup_widget(objet,"fenetre_afficher_client");
...
input1=lookup_widget(objet,"ID");
sortie=lookup_widget(objet,"label662");
strcpy(id,gtk_combo_box_get_active_text(GTK_COMBO_BOX(input1)));
trouve=rechercher_commande(c,id);
gtk_entry_set_text(GTK_ENTRY(input1),"");
...
gdk_color_parse("green",&color);
gtk_widget_modify_fg(sortie,GTK_STATE_NORMAL,&color);
gtk_label_set_text(GTK_LABEL(sortie),"Commande trouvé");
}
else
{
GdkColor color;
...
/*input9=lookup_widget(objet_graphique,"combobox_poste_modifie_employe");

input10=lookup_widget(objet_graphique,"radiobutton_homme_modifie_employe");
input11=lookup_widget(objet_graphique,"radiobutton_femme_modifie_employe");
if(y==1)
{
strcpy(E.sexe,"homme");
}
